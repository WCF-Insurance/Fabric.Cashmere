...
@Component({
    ...
    template: `
        <ng-template let-data let-dialogRef="dialogRef">Cheese {{ localValue }} {{ data?.value }}{{ setDialogRef(dialogRef) }}</ng-template>
    `
})
...
class PizzaMsgComponent {
    constructor(public dialogRef: HcDialogRef<PizzaMsgComponent>, public dialogInjector: Injector, public directionality: Directionality) {}
}
...
@Component({
    template: `
        ...
        <hc-dialog-actions>
            ...
            <button class="close-with-aria-label" aria-label="Best close button ever" [hc-dialog-close]="true">Close</button>
...
describe('DialogService', () => {
    ...
    beforeEach(inject([DialogService, Location, OverlayContainer], (d: DialogService, l: Location, oc: OverlayContainer) => {
        dialog = d;
    ...
    it('should close a dialog and get back a result before it is closed', fakeAsync(() => {
        ...
        const beforeCloseHandler = jasmine.createSpy('beforeClose callback').and.callFake(() => {
            expect(overlayContainerElement.querySelector('hc-dialog-container')).not.toBeNull(
    ...
    it('should close from a ViewContainerRef with OnPush change detection', fakeAsync(() => {
        ...
        expect(overlayContainerElement.querySelectorAll('hc-dialog-container').length).toBe(1, 'Expected one open dialog.');
        ...
        expect(overlayContainerElement.querySelectorAll('hc-dialog-container').length).toBe(0, 'Expected no open dialogs.');
    }));
    ...
    it('should set the proper animation states', () => {
        const dialogRef = dialog.open(PizzaMsgComponent, {viewContainerRef: testViewContainerRef});
        const dialogContainer: DialogContainerComponent = viewContainerFixture.debugElement.query(By.directive(DialogContainerComponent))
            .componentInstance;
    ...
    it('should toggle `aria-hidden` on the overlay container siblings', fakeAsync(() => {
        ...
        expect(overlayContainerElement.hasAttribute('aria-hidden')).toBe(false, 'Expected overlay container not to be hidden.');
    ...
    describe('focus management', () => {
        ...
        it('should focus the first tabbable element of the dialog on open', fakeAsync(() => {
            ...
            expect(document.activeElement!.tagName).toBe('INPUT', 'Expected first tabbable element (input) in the dialog to be focused.');
        }));
        ...
        it('should re-focus trigger element when dialog closes', fakeAsync(() => {
            ...
            expect(document.activeElement!.id).not.toBe('dialog-trigger', 'Expected the focus to change when dialog was opened.');
        ...
        it('should move focus to the container if there are no focusable elements in the dialog', fakeAsync(() => {
            ...
            expect(document.activeElement!.tagName).toBe('HC-DIALOG-CONTAINER', 'Expected dialog container to be focused.');
        }));
    ...
    describe('dialog content elements', () => {
        ...
        function runContentElementTests() {
            ...
            it('should return the [hc-dialog-close] result when clicking the close button', fakeAsync(() => {
                const afterCloseCallback = jasmine.createSpy('afterClose callback');
                dialogRef.afterClosed().subscribe(afterCloseCallback);
            ...
            it('should set the aria-labelledby attribute to the id of the title', fakeAsync(() => {
                ...
                expect(container.getAttribute('aria-labelledby')).toBe(title.id, 'Expected the aria-labelledby to match the title id.');
            }));
        }
    });
...
describe('DialogService with a parent DialogService', () => {
    ...
    it('should close dialogs opened by a parent when calling closeAll on a child DialogService', fakeAsync(() => {
        ...
        expect(overlayContainerElement.textContent!.trim()).toBe(
            '',
            'Expected closeAll on child DialogService to close dialog opened by parent'
        );
    }));

    it('should close dialogs opened by a child when calling closeAll on a parent DialogService', fakeAsync(() => {
        ...
        expect(overlayContainerElement.textContent!.trim()).toBe(
            '',
            'Expected closeAll on parent DialogService to close dialog opened by child'
        );
    }));
...
describe('DialogService with default options', () => {
    ...
    beforeEach(fakeAsync(() => {
        ...
        TestBed.configureTestingModule({
            imports: [DialogModule, DialogTestModule],
            providers: [{provide: HC_DIALOG_DEFAULT_OPTIONS, useValue: defaultConfig}]
        });

        TestBed.compileComponents();
    }));

    beforeEach(inject([DialogService, OverlayContainer], (d: DialogService, oc: OverlayContainer) => {
        dialog = d;
        overlayContainer = oc;
        overlayContainerElement = oc.getContainerElement();
    }));
